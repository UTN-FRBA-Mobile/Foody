...
class RestaurantDataBase(private var context: Context) : SQLiteOpenHelper(
    context,
    DATABASE_NAME, null,
    DATABASE_VERSION
) {
    ...
    fun getDishFromCursor(dishCursor: Cursor, dishId: Int): Dish {
        ...
        val dishDescription =
            dishCursor.getString(dishCursor.getColumnIndexOrThrow(COLUMN_DISH_DESCRIPTION))
        val dishPrice = dishCursor.getDouble(dishCursor.getColumnIndexOrThrow(COLUMN_DISH_PRICE))
        val dishImageId =
            dishCursor.getInt(dishCursor.getColumnIndexOrThrow(COLUMN_DISH_IMAGE_RESOURCE_ID))
        val dishRestaurantId =
            dishCursor.getInt(dishCursor.getColumnIndexOrThrow(COLUMN_DISH_RESTAURANT_ID))
